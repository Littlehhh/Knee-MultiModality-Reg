...
pcl::PointCloud<pcl::PointXYZ>::Ptr
pclICP(PointCloudPtr &moving, PointCloudPtr &fixed, const Eigen::Matrix4f& init_transform = Eigen::Matrix4f::Identity()){
...
//    const char *moving_image_label = "/Users/hui/Data/knee/label_JIANG_XU_femur.nii.gz";
...
//    cor_est.determineCorrespondences(*correspondences);
    ...
    ransac.setInputTarget(ff);
    ransac.setInputCorrespondences (remaining_correspondences);
    ransac.setMaximumIterations(1000);
    ransac.setInlierThreshold(5);
    ransac.setRefineModel(false);
    pcl::CorrespondencesPtr final (new pcl::Correspondences);
    ransac.getRemainingCorrespondences(*remaining_correspondences, *final);
...
//        std::cerr << remaining_correspondences->at (i) << std::endl;
    ...
    pcl::IterativeClosestPoint<pcl::PointXYZ, pcl::PointXYZ> icp;
    icp.setInputSource(mf);
    icp.setInputTarget(ff);
    icp.setMaximumIterations(300);
//    icp.align(*output, transform);
    icp.align(*output, ransac.getBestTransformation());
